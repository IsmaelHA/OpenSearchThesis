...
class LlmEvaluator(Evaluator):
    def evaluate(self, eval_data,k):
        asyncio.run(main_async(eval_data, k))
...
def process_respose(response:Response,log_f,log_message:str,source:str,client=None):
...
async def main_async(eval_df, k):
    # --- Start Overall Timer ---
    ...
    async with aiohttp.ClientSession(connector=conn) as session:
        for i, (index, row) in enumerate(eval_df.iterrows()):
            #print(row)
            task = process_single_alert(session, semaphore, i, row.log_message, row.source,row.label)
            tasks.append(task)
            ...
            if isinstance(result_item, Exception):
                ...
                try:
                    if isinstance(result_item.get("llm_response"),Response):
                        ...
                        process_respose(response,f_out,result_item.get("log_message"),result_item.get("log_message"))
    ...
    analize_results(y_true,y_predict,evaluator="Llm")
